...
/** Ceiling on the queue. An invite link that escapes into a group chat can be
    followed by anyone who sees it, and `pendingRequests` is an embedded array on
    the group document, so unbounded growth is a real shape here rather than a
    theoretical one. Set well below any Mongo document limit: the number that
 ...
 * them to copy, so a wording change stays a wording change — the same reasoning
 * as FriendOutcome in lib/friends.ts. Three are plain successes
 * ("already_participant", "already_pending", "admitted"), one is a success that
 * reads like a refusal ("queued"), and the rest are refusals with genuinely
 * different remedies. */
...
/* Legacy-document guards. The reason is written on membershipOpen in
   models/matching.js: Mongoose applies defaults at creation only. Every group
   that existed before these fields were added has no value for them, so .lean()
...
/**
 * Decides what happens when `userId` tries to join `group`.
 *
 * @param isFriendOfAdmin whether this user is an accepted friend of at least one
 *   current admin, determined by the caller via areFriendsWithAny. Passed in
 *   rather than looked up here so the query can be skipped entirely for the many
 *   outcomes that do not depend on it.
 *
 * The order of the checks is the design. Both "already in" checks run before
 * every state gate, because a member who follows the link again — lost tab,
 * forwarded message, the only URL they kept — must land in their group rather
 * than be told they cannot join a dinner they are already going to.
 */
export function admissionVerdict(
    ...
    /* .toString() on both sides. These ids are typed as strings by
       FriendSummary, but .lean() leaves real ObjectId instances in place, so the
       declared type is a claim the runtime does not honour and comparing them
       raw makes every check fail silently. */
    ...
    /* Orthogonal to status on purpose: the organiser can freeze the guest list
       while the vote is still open. Kept as a separate outcome from the two
       above because it points the joiner at a different action — ask, rather
       than give up. */
    if (rosterLocked(group)) return "roster_locked";

    if (pendingOf(group).length >= PENDING_LIMIT) return "queue_full";

    /* The link gets them to the door; the friendship graph decides whether it
       opens. A stranger who found a forwarded URL lands in the queue, which is
       what stops a leaked link from being a leaked group. */
