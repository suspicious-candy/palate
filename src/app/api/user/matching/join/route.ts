...
/* The code travels in the BODY, not the path. Path segments are written to
   access logs, proxy logs and analytics by default; a POST body is not. The
   invite code is not a password — the friendship check below is what actually
   decides admission — but there is no reason to scatter it either. */
...
/* Refusals only. Each one is a different problem with a different remedy, which
   ...
   permitted to make it — the group's STATE is what is incompatible. Same
   reasoning as the vote route. 404 is the exception, and it is deliberately
   identical for an unknown code and a deleted group so the endpoint cannot be
...
/* Everything that is not a refusal. Note that three of these are successes and
   one — "queued" — is a success that reads like a refusal, so it gets 202
   Accepted: the request was taken, the decision is pending a human. */
    ...
    switch (outcome) {
        ...
        case "already_participant":
            /* Not an error. Someone re-opening the only link they kept must land
               in their group, not be told they cannot join a dinner they are
               already going to. */
            return NextResponse.json({ success: true, state: "joined", group });
        case "queued":
        case "already_pending":
            /* NO group payload. GROUP_POPULATE pulls in participants.approvals
               ...
               hand a stranger everyone's ballots — and the approval step would
               be protecting nothing. Only what the public join page already
               shows. */
...
export const POST = withAuth(async (request, user) => {
    try {
        ...
        /* Narrowed here rather than left to `?.` at every use below — and it is
           also the group_not_found outcome, so one check does both jobs. */
        if (!group) {
            return success("group_not_found", null);
        }

        /* First pass with `false`. isFriendOfAdmin is consulted on the LAST line
           of admissionVerdict and nowhere else, so anything other than "queued"
           coming back means the friendship graph was irrelevant — and we never
           touch the friendship collection at all. That skip is the whole reason
        ...
        /* Admins only. Friend-of-a-participant is deliberately NOT a path: a
           quiet member's entire friend list walking in is not something any
           admin agreed to. Strangers queue — that is what the approval screen
           is for. */
        ...
        /* Compare-and-set. Every precondition lives in the FILTER, not in an
           if-statement above it: the verdict was computed from a group read a
           network round trip ago, and an admin can close the group or lock the
           roster in that window.

           `$ne` on a dotted array path means NO element matches — not "some
           element differs" — which is exactly the guard needed, and it is what
           ...
           membershipOpen uses { $ne: false } rather than true because every
           group created before that field existed has no value for it, and a
           filter of `true` does not match a missing field — see the note on the
           schema. Same for status. */
        const gates = {
            _id: group._id,
            status: { $ne: "closed" },
        ...
        const write =
            verdict === "admitted"
                ...
                : await matchingModel.updateOne(
                      ...
                      /* requestedAt is set here, not defaulted in the schema:
                         the admin queue is sorted by it, and a missing value
                         would sort arbitrarily. The PENDING_LIMIT cap is
                         enforced by the verdict rather than by this filter, so
                         a burst of simultaneous joins can overshoot it by a
                         few — an acceptable slack for a number that exists to
                         bound how many cards a human reads. */
                      { $push: { pendingRequests: { user: userId, requestedAt: new Date() } } }
                  );

        /* matchedCount, not modifiedCount. Zero means a precondition failed
           between the verdict and this write, and the result object cannot say
           which — so re-read and let admissionVerdict explain it. The write is
           the authority; the re-read is what produces an honest message. */
